...
if (!class_exists('AF_themes_info')) {
  class AF_themes_info
  {
    ...
    function __construct()
    {
      ...
      $this->menu_name     = isset($this->config['menu_name']) ? $this->config['menu_name'] : sprintf(esc_html__('%s', 'darknews'), $this->theme_name);
      $this->page_name     = isset($this->config['page_name']) ? $this->config['page_name'] : sprintf(esc_html__('%s', 'darknews'), $this->theme_name);
    ...
    public function darknews_render_templates_kits()
    {
      ...
      if (($darknews_elespare_installed && $darknews_elespare_active) && $plugin_update == 'false' && is_plugin_active($darknews_elementor_installed)) {
        ...
        $class = (!empty($darknews_elespare_verison) && $darknews_elespare_active && $darknews_elespare_verison < $darknews_elespare_old_version)
          ...
          : 'false';
        echo '<div id="templatespare-plugin-install-activate" data-class="' . esc_attr($class) . '" current-theme="elespare" install="' . esc_attr(json_encode($install)) . '" activate="' . esc_attr(json_encode($activate)) . '" page="aft-template-kits" message="' . esc_attr($message) . '" isPro="' . esc_attr($darknews_elespare_status) . '"></div>';
      }
    }
